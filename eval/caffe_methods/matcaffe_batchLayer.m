function [scores,list_im] = matcaffe_batchLayer(list_im, use_gpu, layer)
...
batch_size = 10;
if(strcmp(layer, 'output'))
    dim = 1000; %final output
    model_def_file = '../../models/bvlc_reference_caffenet/deploy.prototxt';
elseif(strcmp(layer, 'fc7'))
    dim = 4096; %fc7
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_4096.prototxt';
elseif(strcmp(layer, 'fc6'))
    dim = 4096; % fc6
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_fc6.prototxt';
elseif(strcmp(layer, 'pool5'))
    dim =9216;%pool5
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_pool5.prototxt';
elseif(strcmp(layer, 'conv5'))
    dim =43264;%conv5
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_conv5.prototxt';
elseif(strcmp(layer, 'conv4'))
    dim =9216;%conv4
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_conv4.prototxt';
elseif(strcmp(layer, 'conv3'))
    dim =9216;%conv3
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_conv3.prototxt';
elseif(strcmp(layer, 'pool2'))
    dim = 43264; %pool2
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_pool2.prototxt';
elseif(strcmp(layer, 'pool1'))
    dim= 69984; %pool1
    model_def_file = '../../models/bvlc_reference_caffenet/deploy_pool1.prototxt';
else
    error('unsupoorted layer');
end
...
if exist('use_gpu', 'var')
  ...
  matcaffe_init(0,model_def_file);
end
...
for bb = 1 : num_batches
    ...
    if(dim==1000||dim==4096) % final or fc7
        scores(:,range) = output_data(:,mod(range-1,batch_size)+1);
    elseif(dim==9216) % pool5
