...
function Result = GetSherlockResults_notharsh_v2(X_embedding, T_embedding, TestData, metric,flag_flexible_map)
...
%         end
    ...
    if(true)
        ...
        Result.KnowledgeDetRatios_Rank  =zeros(1, size(X_embedding,1));
        ...
        for i=1:size(X_embedding,1)
            ...
            for j_ind=1:numel(trueTupleIds)
                ...
                if(trueTupleIds_covered_flag(j)==0)
                    ...
                    if(ismemberAll(j))
                        ...
                        avgRank_i= avgRank_i+ (ismemberAll_ind(j)-offset);
                        numAll = numAll+1;
                        offset = offset+1;
                    end
                else
            ...
            if(numAll==0)
                ...
                MRR_i =MRR_i/numAll;
                avgRank_i = avgRank_i/numAll;
            end
            
            Result.KnowledgeDetRatios_Rank(i) = avgRank_i;
